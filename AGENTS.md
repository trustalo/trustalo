...
1. **Originality.** Trustalo is an independent, permissively-licensed product. No copy-pasted source, prompts, schemas, or migrations from any third-party compliance tool. Inspiration is fine; vendoring is not. Every PR adds the originality attestation in [`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md). CI enforces this through `no-vendored-agpl.yml` and `license-check.yml`.
2. **AI is advisory.** No AI code path may directly mutate a customer record. Every suggestion lands in a `pending` / `draft` state with an audit log entry. See the AI features rule above.
