...
The `packages/integration-manifests` workspace exports declarative **check manifests** — one per connector — whose `checks[]` / `capabilities[]` each declare `controlMappings` (`FrameworkRef`s). These serve two consumers: the custom-integration / "from prompt" pipeline (HTTP probes a user can run without writing a connector), and the **runtime binder** ([`apps/collector/src/integrations/binder`](../apps/collector/src/integrations/binder/index.ts)), which loads a connection's manifest by `integrationId`, unions its FrameworkRefs, and resolves them into the tenant's `IntegrationCheckControl` bindings — this is how a built-in connector's evidence knows which controls it satisfies. Manifests ship for every runtime connector (`aws`, `azure`, `gcp`, `okta`, `auth0`, `github`, `bitbucket`, `google-workspace`, `office365`, `wazuh`) plus `gitlab` (manifest only — no runtime connector yet) and the `endpoint-agent`. Each check maps to every framework it evidences: SOC 2, ISO 27001, Essential Eight, and — as of the HIPAA / PCI DSS 4.0.1 packs — `hipaa` and `pci_dss_4` where a technical control applies (encryption, MFA, access control, logging, malware defence, vulnerability management).
