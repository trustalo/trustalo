...
| `device_malware_detected` | An **active** or **stale** device's endpoint-protection reports malware in its posture (`latestPosture.avDetail`). One alert **per distinct detection** — the dedupe key hashes `product\|signature\|file\|detectedAt`, so a persisting detection alerts once while a genuinely new hit re-alerts — plus a **scan-level fallback** when a scan reports infections but carries no per-detection entries (that key embeds `lastScanAt`). | — |
